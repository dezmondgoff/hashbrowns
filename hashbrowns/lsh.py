...
from hashbrowns.data import BaseData, wrap_data
from hashbrowns.exception_wrap import ExceptionWrapper
...
class LSH(object):
    """
    Abstract class for locality-sensitive hashing objects.
    """
    ...
    def _init_hashtables(self, L, storage_config, storage_fid=None):
        ...
        self.L = L
        self.hash_tables = storage(L, storage_config, storage_fid)
    ...
    def _hash_in_serial(self):
        ...
        _b = self.b
        _data = self.data
        _pack = self.hash_tables.pack_hashes
        _hash = self._hash
    ...
    def _hash_all(self, parallel=True, num_processes=os.cpu_count() + 2,
        ...
        if not parallel:
            ...
            aggregator_args = (self.data, self.hash_tables)
            worker_func = self._hash_in_parallel
            worker_args = listener_args
            self.hash_tables = self._mp_template(listener_args, aggregator_func,
...
#         return result
    ...
    def query(self, y, q, by_id=False, block_size=1e6,
        ...
        """
        Returns `q` nearest neighbors to objects in `y`.
...
class VoroniLSH(LSH):
    """
    A class for locality sensitive hashing based on Eliza et al (2014).
    """
    ...
    def _build(self, block_size):
        if self.L < 1:
            raise ValueError("There must be at least one hash table.")
        if self.k < 1:
